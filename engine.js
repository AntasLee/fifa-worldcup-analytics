// ===== engine.js - 翻译引擎 + 搜索引擎 + 聚合引擎 V1.81 =====
...
if (typeof document !== 'undefined') {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(function() {
            ...
            try { sessionStorage.setItem('engineDataBuilt', 'V1.81'); } catch(e) {}
        }, 500);
    });
}
...
function formatStandardName(type, input, orig) {
    ...
    function _formatPlayerName(p, orig) {
        ...
        if (typeof p === 'object' && p !== null) {
            ...
            // ★ V1.74 修复: 输入已是 "中文名 (English Name)" 格式时，解析提取
            var reStored = /^(.+)\s+\(([^)]+)\)$/;
...
function buildPlayerDetailDOMV3(pd, resolvedKey) {
    ...
    // 如果 playerDB 中没有身价数据，尝试从 squadDB 补充 (V1.74: 优先查_squadValueMap + 单词级匹配)
    if ((latestValue === '—' || !latestValue) && typeof squadDB !== 'undefined' && resolvedKey) {
        // ★ V1.74: 优先从 squadmodal 预存的 _squadValueMap 直接获取（100%准确）
