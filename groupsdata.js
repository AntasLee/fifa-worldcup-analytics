...
function getPlayerKey(displayName) {
    ...
    // ★ V1.70 修复: 去除常见姓名后缀 (对所有输入格式生效，不仅限于括号格式)
    ...
    if (parenIdx >= 0) {
        ...
        // ★ V1.70 修复: 去除常见姓名后缀 (Jr, Sr, II, III, Júnior 等)
        ...
        // ★ V1.70 修复: 尝试交换名姓顺序 ("Hwang In-beom" ↔ "In-beom Hwang")
        ...
        if (enWords.length >= 2) {
            ...
            // ★ V1.70 修复: 连字符名处理 — 尝试去连字符版本 ("In-beom" → "Inbeom")
    ...
    // V1.70 添加最低相关性要求，避免短查询误匹配（如"希门尼斯"误匹配到"Emam Ashour"）
    if (lower.length >= 3) {
        for (var key in playerDB) {
            var pd = playerDB[key];
            // 精确子串匹配
...
window.makePlayerClickable = function(displayName) {
    ...
    // ★ V1.70 修复: 如果 key 为空且 displayName 包含括号，提取英文名作为回退
...
window.searchPlayer = function() {
    ...
    if (matches.length === 1) {
        // V1.70 增强相关性验证：短查询(≤3字符)要求更高相关性，避免子串误匹配
    ...
    // 尝试在比赛数据中搜索 scorer（V1.70: 仅当查询≥3字符且匹配整个单词时才触发，避免误匹配）
